...
function Get-BundledPaths {
    ...
    return @{
        ...
        WkHtmlToPdf = Join-Path $ScriptDir "wkhtmltox\bin\wkhtmltopdf.exe"
    }
}
...
function Test-BundledToolsExist {
    ...
    $wkhtmlExists = Test-Path $Paths.WkHtmlToPdf

    if (-not $pandocExists) {
        Write-Host "Error: Bundled pandoc.exe not found at $($Paths.Pandoc)" -ForegroundColor Red
    }
    if (-not $wkhtmlExists) {
        Write-Host "Error: Bundled wkhtmltopdf.exe not found at $($Paths.WkHtmlToPdf)" -ForegroundColor Red
    }

    return ($pandocExists -and $wkhtmlExists)
}
...
try {
    ...
    if (-not (Test-BundledToolsExist -Paths $ToolPaths)) {
        ...
        Write-Host "    wkhtmltox/bin/wkhtmltopdf.exe" -ForegroundColor Yellow
        exit 1
    }

    Write-Host "Using bundled tools:" -ForegroundColor Gray
    Write-Host "  Pandoc: $($ToolPaths.Pandoc)" -ForegroundColor Gray
    Write-Host "  WkHtmlToPdf: $($ToolPaths.WkHtmlToPdf)" -ForegroundColor Gray
    Write-Host ""
